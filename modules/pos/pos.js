...
class POS {
  ...
  updateTotal() {
    ...
    const shippingAmount = this.orderType === 'delivery' ? (parseFloat(this.shippingAmount) || 0) : 0;
  ...
  _injectOrderTypeUI() {
    ...
    container.innerHTML = `
      <div class="pos-order-type">
        ...
        <div id="pos-delivery-fields" class="pos-delivery-fields">
          ...
          <div class="pos-delivery-field">
            <input type="number" id="pos-delivery-shipping" class="form-input" placeholder="Monto de envío" min="0" step="0.01" value="0">
          </div>
        </div>
      </div>
    `;
    ...
    const shippingInput = document.getElementById('pos-delivery-shipping');

    const saveDelivery = () => {
      this.deliveryName = nameInput?.value || '';
      this.deliveryPhone = phoneInput?.value || '';
      this.deliveryAddress = addressInput?.value || '';
      this.shippingAmount = parseFloat(shippingInput?.value) || 0;
      this.updateTotal();
    };
    ...
    shippingInput?.addEventListener('input', saveDelivery);
  }
