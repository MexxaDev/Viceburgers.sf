...
class BurgerStockService {
    ...
    for (const item of sale.items || []) {
      ...
      for (const ing of recipe.ingredients) {
        ...
        const movement = {
          ...
          action: 'cancellation',
          timestamp: new Date().toISOString(),
          idempotencyKey: `cancel_${sale.id}_${item.productId}_${ing.id}`,
          metadata: {}
        };
        await burgerMovementRepo.create(movement);
        sessionUpdated = true;
  ...
  async getCumulativeConsumed(ingredientId) {
    const movements = await burgerMovementRepo.findAll();
    return movements
      .filter(m => m.ingredientId === ingredientId && m.action === 'sale')
      .reduce((sum, m) => sum + Math.abs(m.delta), 0);
  }

  async getCumulativeProductRanking() {
    const movements = await burgerMovementRepo.findAll();
    const saleMovements = movements.filter(m => m.action === 'sale');
    const ranking = {};
    for (const m of saleMovements) {
